...
test('attachCachedRoutes copies origin name and ICAO', () => {
  _resetRouteCacheForTests();
  _seedRouteCacheForTests('RYR35DU', { ...KUN, originName: 'Málaga' });
  const [ac] = attachCachedRoutes([
    { callsign: 'RYR35DU', lat: POS.lat, lon: POS.lon, track: POS.track },
  ]);
  assert.equal(ac.originIata, 'AGP');
  assert.equal(ac.originIcao, 'LEMG');
  assert.equal(ac.originName, 'Málaga');
});
